    ...
    return (
        <Accordion type="single" collapsible className="w-full md:max-w-[65%]">
            {services.map((service) => (
                <AccordionItem key={service.id} value={service.id} className={service.id === 'item-4' ? 'border-b-0' : ''}>
                    <AccordionTrigger className="hover:no-underline">
                        <div className="flex gap-4 items-end">
                            <Image width={32} height={32} src={service.icon} alt={`${service.title}-icon`} />
                    ...
                    <AccordionContent className="flex flex-col gap-4 md:ps-6">
                        ...
                        <CustomButton
                            ...
                            className="text-sm p-1 aspect-square w-fit h-fit bg-white text-primary rounded-full hover:bg-white/90 font-semibold"
                        />
                    </AccordionContent>
                </AccordionItem>
            ))}
